...
const ImageSlider = ({ urls }: ImageSliceProps) => {
  console.log(urls)
    ...
    <div className="group relative bg-zinc-100 aspect-square overflow-hidden rounded-xl">
      <div className="absolute z-10 inset-0 opacity-0 group-hover:opacity-100 transition">
        ...
        <button onClick={(e)=> {
          ...
          [inactiveStyle]:slideConfig.isBegginning, "hover:bg-primary-300 text-primary-800 opacity-100" :!slideConfig.isBegginning
       ...
       onSwiper={(swiper)=> setSwiper(swiper)} spaceBetween={50} slidesPerView={1} modules={[Pagination]} className="h-full w-full">
        {urls.map((url,i)=>(
          <SwiperSlide
           ...
           key={i} className="-z-10 relative h-full w-full">
